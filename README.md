# Hello 👋 I'm Balakiruthika R

---

## 💡 About Me

I am a passionate **Data Analyst** with a strong interest in turning raw data into meaningful insights. I specialize in analyzing datasets, building visualizations, and applying machine learning techniques to solve real-world problems.

My journey involves continuous learning, working on data-driven projects, and exploring how analytics can support better decision-making. I believe in the power of curiosity, consistency, and hands-on practice to build a strong foundation in tech.

📍 Coimbatore, Tamil Nadu, India  
🎓 Rathinam Technical Campus  
🎯 Areas of Interest: Data Analysis, Machine Learning, Business Intelligence, Data Visualization, Cloud Computing, Frontend Development

---

## 🛠️ Skills & Tools

![Python](https://img.shields.io/badge/Python-blue?style=for-the-badge&logo=python)
![Java](https://img.shields.io/badge/Java-orange?style=for-the-badge&logo=java)
![NumPy](https://img.shields.io/badge/NumPy-blue?style=for-the-badge&logo=numpy)
![Pandas](https://img.shields.io/badge/Pandas-purple?style=for-the-badge&logo=pandas)
![Excel](https://img.shields.io/badge/Excel-green?style=for-the-badge&logo=microsoft-excel)
![SQL](https://img.shields.io/badge/SQL-lightgrey?style=for-the-badge&logo=postgresql)
![Power BI](https://img.shields.io/badge/PowerBI-yellow?style=for-the-badge&logo=powerbi)
![Machine Learning](https://img.shields.io/badge/MachineLearning-red?style=for-the-badge)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-orange?style=for-the-badge&logo=scikit-learn)

---

## 📊 Featured Projects

### 📁 Data Analysis Projects

| Project Name | Live Link | Description | Tools Used |
|-------------|----------|-------------|-------------|
| Customer Behavior Analysis | [Repo](https://github.com/balakiruthika12/customer_behavior_analysis) | Extract meaningful insights and present findings in a clear and business-ready format | Python,Pandas,Numpy,SQL,PowerBI |
| Food Delivery | [Repo](https://github.com/balakiruthika12/food-delivery-sql-project) | Analytical queries to extract meaningful business insights | SQL |
| E-Commerce Sales Analysis | [Repo](https://github.com/balakiruthika12/E-Commerce-Sales-Analysis) | Show sales trends, top products, customers, and country-wise revenue | Excel,SQL |
| Adidas Sales Analysis | [Repo](https://github.com/your-username/data-cleaning) | From data cleaning and transformation to SQL analysis and interactive dashboard visualization | Python,Pandas,Matplotlib,SQL,PowerBI |

---

# 🤖 Machine Learning Projects

| Project Name | Live Link | Description | Tools Used |
|---|---|---|---|
| House Price Prediction | [App Link](https://house-price-nb3xqpyex6k9jwrdyldtt7.streamlit.app/) | Built a machine learning model to predict house prices based on features like area, bedrooms, and bathrooms with a Streamlit web application for real-time predictions | Python, Pandas, NumPy, Scikit-Learn, Streamlit, Joblib |

---

## 📈 What I Do

✔️ Data Cleaning & Preprocessing  
✔️ Data Visualization & Dashboard Creation  
✔️ Statistical Analysis  
✔️ Machine Learning Model Building  
✔️ SQL Query Optimization  
✔️ Business Insights Generation  

---

## 🌱 Currently Learning

- Advanced Machine Learning  
- Data Engineering Basics  
- Cloud (AWS for Data Analytics)  

---

## 📫 Connect With Me

- 💼 LinkedIn: [https://www.linkedin.com/in/bala-kiruthika-a03745377/]
- 📧 Email: [r.balakiruthika@gmail.com]

---

⭐ *“Turning data into insights and insights into decisions.”*
